...
export default function GoogleLoginButton() {
  ...
  const googleAuthMutation = useMutation({
    ...
    onError: (error: any) => {
      alert(error.response?.data?.detail || 'Google authentication failed');
    },
  });

  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      // Note: Google returns access_token, but backend expects idToken
      // We'll need to fetch the ID token from Google
      try {
